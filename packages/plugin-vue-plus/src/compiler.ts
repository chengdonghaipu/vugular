...
export class Compiler {
    ...
    private parseParamTypes(parameters: ts.NodeArray<ts.ParameterDeclaration>, onlyOptional = false): string[] {
        return parameters.map(param => {
            ...
            let injectOptions = decoratorList.reduce((previousValue, currentValue) => Object.assign(previousValue, currentValue.flag), {})
                ...
                switch (name) {
                    ...
                    case 'styleUrls':
                        if (ts.isArrayLiteralExpression(prop.initializer)) {
                            this.styleUrls = prop.initializer.elements.map((e) =>
                                e.getText().slice(1, -1)
                            )
                        }
                        break
                    case 'styles':
                        if (ts.isArrayLiteralExpression(prop.initializer)) {
                            this.styles = prop.initializer.elements.map((e) =>
                                e.getText().slice(1, -1)
                            )
                        }
                        break
                    case 'providers':
                        ...
                        this.providers = prop.initializer.elements.map((element) => {
                                if (ts.isObjectLiteralExpression(element)) {
                                    const provideNode = element.properties.find(prop => ts.isIdentifier(prop.name!) && prop.name.escapedText === 'provide');
                                    const useClassNode = element.properties.find(prop => ts.isIdentifier(prop.name!) && prop.name.escapedText === 'useClass');
                                    // @ts-ignore
                                    if (provideNode && ts.isIdentifier(provideNode.initializer) && useClassNode && ts.isIdentifier(useClassNode.initializer)) {
                                        // @ts-ignore
    ...
    generate(id: string): string {
        ...
        if (styleUrls) {
            styleUrls.push(...styleUrls.map((v: string) => {
                const basePath = path.dirname(id)
