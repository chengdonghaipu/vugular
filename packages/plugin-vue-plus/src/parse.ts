    ...
    if (ts.isClassDeclaration(node)) {
        ...
        node.members.forEach((member) => {
            if (ts.isPropertyDeclaration(member)) {
                ...
                output.paramTypes = member.parameters.map(param => {
                    ...
                    return `{
                        token: ${token},
                        injectOptions: ${JSON.stringify(decoratorList.reduce((previousValue, currentValue) => Object.assign(previousValue, currentValue.flag), {}))}
                    }`
                }).filter(value => !!value)
            }
        })
                ...
                arg.properties.forEach((prop) => {
                    if (ts.isPropertyAssignment(prop)) {
                        ...
                        switch (name) {
                            ...
                            case 'styles':
                                if (ts.isArrayLiteralExpression(prop.initializer)) {
                                    output['styles'] = prop.initializer.elements.map((e) =>
                                        e.getText().slice(1, -1)
                                    )
                                }
                                break
